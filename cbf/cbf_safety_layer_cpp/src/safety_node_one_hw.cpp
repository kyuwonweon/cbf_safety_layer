...
public:
    SafetyNode() : Node("safety_node_cpp") {
        ...
        // HARDWARE: subscribe to FrankaRobotState instead of JointState
...
private:
    ...
    Eigen::VectorXd v_manual_command, v_teleop_command;
    Eigen::VectorXd q_min, q_max, v_limit;
            ...
            if (other_robot_active) {
                for (const auto& [name_, cap_] : capsules_other) {
                    ...
                    Eigen::Vector3d start_point =
                        data_other.oMf[id_start].translation() + other_base_offset - base_offset;
                    Eigen::Vector3d end_point =
                        data_other.oMf[id_end].translation() + other_base_offset - base_offset;
    ...
    void apply_limits(Eigen::MatrixXd& C, Eigen::VectorXd& L, Eigen::VectorXd& U,
        ...
        // HARDWARE: conservative acceleration cap
        double a_max = 2.0;
        for (int i = 0; i < nv_self; ++i) {
            if (row >= C.rows()) break;
            C(row, i) = 1.0;
    ...
    void publish_markers() {
        ...
        for (const auto& [name, cap] : capsules_self) {
            ...
            Eigen::Vector3d p1 = p1_raw, p2 = p2_raw;
