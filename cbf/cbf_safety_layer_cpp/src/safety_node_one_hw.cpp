...
public:
    SafetyNode() : Node("safety_node_one") {
        ...
        sub_js_ = this->create_subscription<franka_msgs::msg::FrankaRobotState>(
            "/franka_robot_state_broadcaster/robot_state", qos,
            std::bind(&SafetyNode::joint_cb, this, std::placeholders::_1));
...
private:
    ...
    // HARDWARE: FrankaRobotState subscription
    ...
    // HARDWARE: reads from FrankaRobotState measured_joint_state
    void joint_cb(const franka_msgs::msg::FrankaRobotState::SharedPtr msg) {
        ...
        if (!initial_pose_set) {
            ...
            size_t limit = std::min((size_t)nq_, msg->measured_joint_state.position.size());
        ...
        for (const auto& [name, capsule] : capsules_) {
            ...
            if (h_obs < 0.4) {
                ...
                double kp = 20.0;
                double kd = 5.0;
                double l_kin_obs = std::clamp(-kp * h_obs - kd*h_dot_obs, -10.0, 10.0);
        ...
        try {
            ...
            if (qp_->results.info.status == proxsuite::proxqp::QPSolverOutput::PROXQP_SOLVED) {
                ...
                // HARDWARE: reduced acceleration cap
                a_safe = a_safe.cwiseMin(2.0).cwiseMax(-2.0);
                v_safe_ += a_safe*dt;
                for(int i=0; i<nv_; ++i) {
        ...
        double dt) 
    {
        ...
        // HARDWARE: reduced max acceleration
        double a_max = 2.0;
